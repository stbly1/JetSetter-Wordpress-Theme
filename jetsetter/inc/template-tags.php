...
if ( ! function_exists( 'ashland_dispatch_paging_nav' ) ) :
/**
 * Display navigation to next/previous set of posts when applicable.
 *
 * @since Ashland Dispatch 1.0
 */
function ashland_dispatch_paging_nav() {
	// Don't print empty markup if there's only one page.
	if ( $GLOBALS['wp_query']->max_num_pages < 2 ) {
		return;
	}
	...
	<nav class="navigation post-navigation" role="navigation">
		...
		<div class="nav-links">
			...
			if ( is_attachment() ) :
				...
				next_post_link( '%link', __( '<span class="meta-nav">Read Next Post </span>%title<br/>', 'twentyfourteen' ) );
				previous_post_link( '%link', __( '<span class="meta-nav">Read Previous Post </span>%title', 'twentyfourteen' ) );
...
if ( ! function_exists( 'twentyfourteen_page_nav' ) ) :
/**
 * Display navigation to next/previous post when applicable.
 *
 * @since Twenty Fourteen 1.0
 */
function twentyfourteen_page_nav() {
	// Don't print empty markup if there's nowhere to navigate.
	$previous = ( is_attachment() ) ? get_post( get_post()->post_parent ) : get_adjacent_post( false, '', true );
	$next     = get_adjacent_post( false, '', false );
	...
	<nav class="navigation post-navigation" role="navigation">
		...
		<div class="nav-links">
			...
			if ( is_attachment() ) :
				...
				next_post_link( '%link', __( '<span class="meta-nav">Visit Next Page </span>%title<br/>', 'twentyfourteen' ) );
				previous_post_link( '%link', __( '<span class="meta-nav">Visit Previous Page </span>%title', 'twentyfourteen' ) );
